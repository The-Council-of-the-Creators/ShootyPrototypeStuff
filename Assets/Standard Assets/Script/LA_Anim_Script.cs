...
public class LA_Anim_Script : WeaponBase
{
    ...
    private IEnumerator BeginFireCooldown()
    {
        if (isCoolingDown)
            yield break;
        isCoolingDown = true;
        yield return new WaitForSeconds(GetAnimationClipLength("LA_Classic_Skeleton|LA_Classic_Fire"));
        isCoolingDown = false;
        cooldown = null;
    }
    ...
    private IEnumerator PlaySelectAnimation()
    {
        ...
        yield return new WaitForSeconds(GetAnimationClipLength("LA_Classic_Skeleton|LA_Classic_Select"));
        selecting = false;
    }
}
